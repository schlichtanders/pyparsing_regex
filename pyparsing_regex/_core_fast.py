...
from schlichtanders.myobjects import Count, FastStructure
...
class ParserElement(ParserElementType):
    ...
    def __init__(self, pattern, silent=False):
        if silent:
            ...
            self.structure = FastStructure()
            self.pattern = pattern
            self.name = self.pattern
        else:
            # create Count() Structure
            self.structure = FastStructure(Count())
    ...
    def _parseString(self, instring):
        ...
        Count.reset()
        struct = deepcopy(self.structure) # TODO this might get faster by using json serialization
